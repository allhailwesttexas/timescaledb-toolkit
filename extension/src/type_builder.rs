    ...
    (
        ...
        ::paste::paste! {
            ...
            impl<'input> $name<'input> {
                ...
                pub unsafe fn cached_datum_or_flatten(&mut self) -> pgrx::pg_sys::Datum {
                    ...
                    match self.1 {
                        ...
                        FromInput(bytes) | Flattened(bytes) => pgrx::pg_sys::Datum::from(bytes.as_ptr()),
                    }
                }
            }
            ...
            impl<$lifetemplate> [<$name Data>] $(<$inlife>)? {
                ...
                pub fn to_pg_bytes(&self) -> &'static [u8] {
                    ...
                    unsafe {
                        ...
                        let memory: *mut MaybeUninit<u8> = pgrx::pg_sys::palloc0(len).cast();
            ...
            impl<$lifetemplate> pgrx::FromDatum for $name<$lifetemplate> {
                unsafe fn from_polymorphic_datum(datum: pgrx::pg_sys::Datum, is_null: bool, _: pgrx::pg_sys::Oid) -> Option<Self>
                where
                    Self: Sized,
                {
                    use flat_serialize::FlatSerializable as _;
                    ...
                    let mut ptr = pgrx::pg_sys::pg_detoast_datum_packed(datum.cast_mut_ptr());
                    //TODO is there a better way to do this?
                    if pgrx::varatt_is_1b(ptr) {
                        ptr = pgrx::pg_sys::pg_detoast_datum_copy(ptr);
                    }
            ...
            impl<$lifetemplate> pgrx::IntoDatum for $name<$lifetemplate> {
                fn into_datum(self) -> Option<pgrx::pg_sys::Datum> {
                    ...
                    let datum = match self.1 {
                        Flattened(bytes) => pgrx::pg_sys::Datum::from(bytes.as_ptr()),
                        FromInput(..) | None => pgrx::pg_sys::Datum::from(self.0.to_pg_bytes().as_ptr()),
                    };
                    Some(datum)
                }
    ...
    ($state: ident, version: $version: expr) => {
        {
            ...
            $crate::raw::bytea::from(pgrx::pg_sys::Datum::from(writer.into_inner().as_mut_ptr()))
        }
    };
}
#[macro_export]
macro_rules! do_deserialize {
    ($bytes: expr, $t: ty) => {{
        ...
        let state: $t = unsafe {
            ...
            let detoasted = pgrx::pg_sys::pg_detoast_datum_packed(input.cast_mut_ptr());
